...
#### **Question:** Explain the role of scale-space representation in computer vision. Discuss: a) why analyzing an image at multiple scales can be useful, b) how Gaussian filtering is used to construct a scale-space representation, c) the difference between smoothing at a single scale and building a Gaussian pyramid, d) one application where multi-scale analysis is important.
